...
export async function enqueueSydFitTask(config, endpoint, payload) {
  ...
  const { gcpProjectId, gcpLocation, gcpQueueName, sydFitServiceUrl, sydFitApiKey } = config;

  if (!gcpProjectId || !gcpLocation || !gcpQueueName || !sydFitServiceUrl) {
    throw new Error("Cloud Tasks configuration is incomplete. Missing GCP_PROJECT_ID, GCP_LOCATION, GCP_QUEUE_NAME, or SYDFIT_SERVICE_URL.");
  }
  ...
  try {
    ...
    writeLog("INFO", "Cloud Task enqueued successfully", { taskName: response.name, targetUrl: url });
    return response;
  } catch (error) {
