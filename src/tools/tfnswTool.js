...
export const getTfnswAlertsTool = (config) =>
	tool({
		...
		description:
			"Fetches real-time Transport for NSW (TfNSW) service alerts for a single transport mode. Call this once per relevant mode (e.g. once for 'train', once for 'lightrail') if the user commutes on multiple modes.",
		parameters: z.object({
			mode: z
