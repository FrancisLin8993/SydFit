    ...
    const result = await startActiveObservation("process-task", async (span) => {
        return propagateAttributes({ userId: "francis", tags: ["ask"] }, async () => {
            try {
                ...
                const result = await runner.run(agent, JSON.stringify({ input: query }));
                ...
                writeLog("INFO", `[Process Task API] ✅ Processing complete (handled by: ${handledBy}), triggering Bark push notification...`);
